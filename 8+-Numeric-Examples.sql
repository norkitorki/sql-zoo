...
CREATE TABLE nss (
  ...
  level, TEXT,
  question, TEXT,
  A_STRONGLY_DISAGREE	INTEGER,
  A_DISAGREE INTEGER,
  A_NEUTRAL	INTEGER,
  A_AGREE	INTEGER,
  A_STRONGLY_AGREE INTEGER,
  A_NA INTEGER,
  CI_MIN INTEGER,
  score	INTEGER,
  CI_MAX INTEGER,
...
(SELECT SUM(sample) FROM nss WHERE question= 'Q01' AND institution = a.institution AND subject LIKE '%Computer%' GROUP BY a.institution) AS comp
FROM nss a
