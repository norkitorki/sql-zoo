...
CREATE TABLE teacher (id INTEGER PRIMARY KEY, dept INTEGER, name TEXT, phone INTEGER, mobile TEXT);

CREATE TABLE dept (id INTEGER PRIMARY KEY FOREIGN KEY, name TEXT);
...
SELECT name, (CASE WHEN dept BETWEEN 1 AND 2 THEN 'Sci' ELSE 'Art' END)
FROM teacher;
...
SELECT name, 
(CASE WHEN dept BETWEEN 1 AND 2 THEN 'Sci' WHEN dept = 3 THEN 'Art' ELSE 'None' END)
FROM teacher;
